class Grafo(object):
    ...
    def __init__(self, orientado = False):
        self.n, self.m, self.orientado = None, None, None
    def DefinirN(self, n):
    ...
    def E(self, IterarSobreNO = False):
        #Retornar a lista de aresta uv, onde u é um inteiro
        #e v é um inteiro se o grafo é GrafoMatrizAdj
        #ou IterarSobreNo=False; v é GrafoListaAdj.NoAresta,
        #caso contrário.
        for v in self.V():
...
class GrafoMatrizAdj(Grafo):
    ...
    def N(self, v, Tipo = "*", Fechada = False, IterarSobreNo = False):
        #Retonr lista de vértices vizinhos do vértice v. Se Fechada=True, o próprio v é incluido na lista.
...
class GrafoListaAdj(Grafo):
    class NoAresta(object):
        ...
        def __init__(self):
            ...
            self.Prox = None
        def Aresta(object):
            #Objeto único para respresentar aresta.
            #Atributos:
            ...
            def DefinirN(self, n, VizinhancaDuplamenteLigada = False):
                #Define o número n de vértices.
                #Se VizinhancaDuplamenteLIgada=True, a lista encadeada dos vizinhos de m vértice é duplamente ligada (permitindo remoção de arestas de tempo constante).
                super(grafoListaAdj, self).Definir(n)
                self.L = [None]*(self.n+1)
                for i in range(1, self.n+1):
                    self.L[i] = GrafoListaAdj.NoAresta() #nó cabeça
                    self.VizinhancaDuplamenteLigada = VizinhancaDuplamenteLigada
            def AdicionnarAresta(self, u, v):
                #Adionar aresta uv.
                def AdicionarLIsta(u, v, e, Tipo):
                    ...
                    if self.VizinhancaDuplamenteLigada:
                        ...
                        if self.L[u].Prox.Prox != None:
                            self.L[u].Prox.Ant = self.L[u].Prox
                    if self.orientado:
                        No.Tipo = Tipo
                    return No
                ...
                e.v1, e.v2 = u, v
                e.No1 = AdionarLista(u, v, e, "+")
                e.No2 = AdionarLIsta(v, u, e, "-")
                self.m = self.m+1
                return e
        ...
        def SaoADj(self, u, v):
            ...
            Tipo = "+" of self.orientado else "*"
            for w in range(u, Tipo):
                if w == v:
                    return True
            return False

        def N(self, v, Tipo = "*", Fecahda = False, IterarSobreNo = False):
            #Retorna lista de Grafo.noAresta representando os vizinhos do vértice v.
            #Se Fechada=True, o próprio v é incluido na lista.
            #Tipo="*" significa listar apenas as arestas incidentes em v. Se G é orientado.
            #Tipo="+" (resp. "-") significa listar apenas as arestas de saída (resp. entrada) de v.
            #IterarSobreNo=False indica que a lista de vizinhos deve constituir da lista de vétices. Caso contrário,
            #caso contrário, a lista é dos nós da lista encadeada de vizinhos (NoAresta).
            if Fechada:
                No = GrafoListaAdj.NoAresta()
                No.Viz, No.e, No.Prox = v, None, None
                yield No if InterarSobreNo else No.Viz
                w = self.L[v].Prox
                while w != None:
